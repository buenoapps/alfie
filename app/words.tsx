...
import { getWords } from '@/constants/words';
    ...
    >
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.grid}>
          {words.map(({ id, word, emoji, color }) => (
            <View key={id} style={styles.cell}>
              <WordTile
                word={word}
                emoji={emoji}
                color={color}
                onPress={() => router.push(`/word/${id}`)}
              />
            </View>
          ))}
        </View>
        ...
        <Pressable
          accessibilityRole="button"
          accessibilityLabel={t('playWords')}
          onPress={startQuiz}
          style={({ pressed }) => [styles.playButton, pressed && styles.playButtonPressed]}
        >
          <ThemedText style={styles.playEmoji}>🎮</ThemedText>
          <ThemedText style={styles.playLabel}>{t('playWords')}</ThemedText>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  );
