...
import { getWords, type WordEntry } from '@/constants/words';
...
function buildRound(pool: WordEntry[]): Question[] {
  const picks = shuffle(pool).slice(0, Math.min(ROUND_SIZE, pool.length));
  return picks.map((answer) => {
    const distractors = shuffle(pool.filter((w) => w.id !== answer.id))
...
export default function WordQuizScreen() {
  ...
  const pool = getWords(lang);
  const [questions, setQuestions] = useState<Question[]>(() => buildRound(pool));
  ...
  const speakWord = useCallback(
    (entry: WordEntry) => {
      ...
      Speech.stop();
      Speech.speak(entry.word, { rate: 0.85, pitch: 1.05, language: speechLocale(lang) });
    },
    [audioEnabled, lang]
  );
  ...
  const speakChoice = useCallback(
    (word: string) => {
      if (!audioEnabled) return;
      Speech.stop();
      Speech.speak(word, { rate: 0.85, pitch: 1.05, language: speechLocale(lang) });
    },
    [audioEnabled, lang]
  );
  ...
  const handleChoice = (word: string) => {
    ...
    const correct = word === current.answer.word;
  ...
  return (
    <Animated.View entering={FadeIn.duration(300)} style={styles.questionContent}>
      ...
      <View style={styles.optionsGrid}>
        {options.map((word) => (
          <View key={word} style={styles.optionCell}>
            <OptionButton
              word={word}
              wrong={wrongChoices.has(word)}
              correct={correctChoice === word}
...
function OptionButton({ word, wrong, correct, disabled, onPress }: OptionButtonProps) {
    ...
    <Animated.View style={animatedStyle}>
      ...
      >
        <ThemedText
          style={[
            styles.optionWord,
            wrong && styles.optionWordWrong,
            correct && styles.optionWordCorrect,
          ]}
        >
          {word}
        </ThemedText>
